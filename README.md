...
**Linux (recommended):** Use the FinTV-ready Jellyfin Docker image [`ghcr.io/binarygeek119/jellyfin-unstable-fintv:unstable`](https://github.com/binarygeek119/jellyfin-unstable-fintv). It includes Docker CLI for WeatherStar and Playwright sidecar control, plus `yt-dlp` and `fpcalc` for CommercialBrainz and audio fingerprinting. Mount the Docker socket (`/var/run/docker.sock`) and add `group_add` for the socket GID.
