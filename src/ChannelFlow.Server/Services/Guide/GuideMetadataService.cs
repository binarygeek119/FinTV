...
public class GuideMetadataService
{
    ...
    private async Task<string?> CacheJellyfinPosterAsync(Guid itemId, CancellationToken cancellationToken)
    {
        ...
        var (jellyfinUrl, jellyfinToken) = ResolveJellyfinEndpoint();
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(jellyfinToken))
            {
                ...
                request.Headers.TryAddWithoutValidation(
                    "Authorization",
                    "MediaBrowser Client=\"ChannelFlow\", Device=\"ChannelFlow-Server\", DeviceId=\"channelflow\", Version=\"1.0\", Token=\"" + jellyfinToken.Replace("\"", "", StringComparison.Ordinal) + "\"");
            }
    ...
    private (string? Url, string? Token) ResolveJellyfinEndpoint()
    {
        try
        {
            var row = _db.MediaServerConnections.AsNoTracking()
                .Where(c => c.Kind == MediaServerKind.Jellyfin && c.Enabled)
                .OrderBy(c => c.SortOrder)
                .FirstOrDefault();
            if (row is not null && !string.IsNullOrWhiteSpace(row.BaseUrl))
            {
                return (row.BaseUrl.Trim().TrimEnd('/'), row.AccessToken);
            }
        }
        catch (Exception)
        {
        ...
        var plugin = FinTvRuntime.Current?.Configuration.JellyfinPluginUrl?.Trim().TrimEnd('/');
        if (!string.IsNullOrWhiteSpace(plugin))
        {
            return (plugin, PluginApiKey.Resolve());
        }

        var env = Environment.GetEnvironmentVariable("JELLYFIN_URL")?.Trim().TrimEnd('/');
        return (string.IsNullOrWhiteSpace(env) ? null : env, null);
    }
