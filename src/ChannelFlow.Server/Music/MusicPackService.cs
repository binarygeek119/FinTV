...
public sealed class MusicPackService
{
    ...
    public async Task EnsureAnytimeDownloadedAsync(CancellationToken cancellationToken)
    {
        ...
        if (IsAutoDownloadSkipped(AnytimeId))
        {
            _logger.LogInformation("Anytime music pack auto-download skipped; it was removed on this server");
            return;
        }
    ...
    public Task DownloadAsync(string packId, CancellationToken cancellationToken)
    {
        ...
        SetAutoDownloadSkipped(pack.Id, skipped: false);
    ...
    public void Delete(string packId)
    {
        var pack = FindPack(packId)
            ?? throw new InvalidOperationException("Unknown music pack.");
        if (string.Equals(pack.Id, AnytimeId, StringComparison.OrdinalIgnoreCase))
        {
            SetAutoDownloadSkipped(pack.Id, skipped: true);
        }
    ...
    private async Task RunDownloadAsync(MusicPackDefinition pack, PackJob job, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (IsAutoDownloadSkipped(pack.Id))
            {
                job.Status = "idle";
                job.Error = null;
                _logger.LogInformation("Music pack {PackId} download discarded because it was removed", pack.Id);
                return;
            }
    ...
    private MusicPackStatus Describe(MusicPackDefinition pack, string? activeId)
    {
        ...
        return new MusicPackStatus
        {
            ...
            AutoDownloadSkipped = IsAutoDownloadSkipped(pack.Id)
        };
    }
    ...
    private static string AutoDownloadSkipPath(string packId)
        => Path.Combine(MusicRoot(), packId + ".skip-auto");

    private static bool IsAutoDownloadSkipped(string packId)
        => File.Exists(AutoDownloadSkipPath(packId));

    private static void SetAutoDownloadSkipped(string packId, bool skipped)
    {
        var path = AutoDownloadSkipPath(packId);
        if (skipped)
        {
